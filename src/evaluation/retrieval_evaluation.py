...
class QueryContextGenerator:
    ...
    def __init__(self, llm: LLM, system_message: str = None, user_message: str = None):
        self.llm = llm
        self.reranker = ReRanker()
        self.system_message = system_message
        self.user_message = user_message

    def _clean_validate_data(
        self,
        data: list[dict],
    ...
    def train_val_split(
        ...
        if end_index > len(clean_data):
            raise ValueError(
                "Cannot create dataset with desired number of questions, try using a larger dataset"
            )
    ...
    def generate_qa_embedding_pairs(
        ...
        while len(question_bank) < num_total_questions:
            ...
            doc_id = chunk["doc_id"]
            counter += 1
            assist_message = generate_prompt_tmpl.format(
                summary=summary,
                guest=guest,
            ...
            if threshold:
                ...
                scores = self.reranker.predict(
                    sentences=pairs, activation_fct=self.reranker.activation_fct
                )
                ...
                if len(question_bank) < num_total_questions:
                    ...
                    for question in questions:
                        if len(question_bank) < num_total_questions:
                            ...
                            queries[question_id] = (
                                question[0]
                                if isinstance(question, (np.ndarray, list))
                                else question
                            )
            ...
            if (
                len(question_bank) % int((num_total_questions * 0.2)) == 0
                and len(question_bank) != 0
            ):
    ...
    def generate_retrieval_dataset(
        ...
        system_message = (
            ...
            else "You are an expert at generating questions from a given text."
        )
        while len(question_bank) < num_total_questions:
            chunk = clean_data[counter]
            counter += 1
            ...
            transcript = chunk["content"]
            doc_id = chunk["doc_id"]
            user_message = dataset_generation_prompt.format(
                guest=guest,
                title=title,
                transcript=transcript,
            ...
            if not any(questions):
                ...
                valid_user_message = qa_validation_prompt.format(
                    title=title, transcript=transcript, question=question
                )
                try:
                    valid_response = self.llm.chat_completion(
                ...
                if valid_response.strip() == "1":
                    ...
                    progress.update(1)
                    if (
                        flavor_counter % quarter == 0
                        and flavor_counter != num_total_questions
                    ):
                        qa_flavor += 1
                        logger.info(
                            f"Changing QA Flavor: at count {flavor_counter}, using qa_flavor {qa_flavor}"
                        )
    ...
    def generate_qa_triplets(
        ...
        """
        system_message = (
            self.system_message if self.system_message else default_system_message
        )
        user_message = (
            self.user_message if self.user_message else qa_triplet_generation_prompt
        )
        if user_message != qa_triplet_generation_prompt:
            logger.warning(
                'You are using a custome user message, which will require a "guest" and "transcript" field in your prompt.'
            )
        valid_json_triplet = []
        clean_data = self._clean_validate_data(data, total_chars=total_chars)
        random.shuffle(clean_data)
    ...
    def _check_valid_keys(
        self, sample: dict, valid_keys: list[str] = ["positive", "hard_negative"]
    ) -> list[dict]:
        if len(sample) != 2:
            return False
...
def execute_evaluation(
    ...
    results_dict = (
        add_params(results_dict, user_def_params) if user_def_params else results_dict
    )
    ...
    for query_id, q in tqdm(dataset["queries"].items(), "Queries"):
        ...
        try:
            if "hybrid" in search_type:
                hybrid_doc_ids, hybrid_response = get_doc_ids(
                    "hybrid",
                    retriever,
                    q,
                    collection_name,
            ...
            if "kw" in search_type:
                kw_doc_ids, kw_response = get_doc_ids(
                    "kw",
                    retriever,
                    q,
                    collection_name,
            ...
            if "vector" in search_type:
                vector_doc_ids, vector_response = get_doc_ids(
                    "vector",
                    retriever,
                    q,
                    collection_name,
            ...
            if not hit:
                ...
                if "hybrid_response" in locals():
                    hybrid_miss_info = _create_miss_info(
                        "hybrid", q, hybrid_response, dataset, doc_id
                    )
                    response_misses.append(hybrid_miss_info)
                if "kw_response" in locals():
                    kw_miss_info = _create_miss_info(
                        "kw", q, kw_response, dataset, doc_id
                    )
                    response_misses.append(kw_miss_info)
                if "vector_response" in locals():
                    vector_miss_info = _create_miss_info(
                        "vector", q, vector_response, dataset, doc_id
                    )
...
def calc_hit_rate_scores(
    ...
    for prefix in search_type:
        if prefix in accepted_search_types:
            results_dict[f"{prefix}_hit_rate"] = round(
                results_dict[f"{prefix}_hit_rate"] / results_dict["total_questions"], 2
            )
...
def calc_mrr_scores(
    ...
    for prefix in search_type:
        if prefix in accepted_search_types:
            results_dict[f"{prefix}_mrr"] = round(
                results_dict[f"{prefix}_mrr"] / results_dict["total_questions"], 2
            )
...
def get_doc_ids(
    ...
    if search_mode == "hybrid":
        ...
        response = retriever.keyword_search(
            request=query,
            collection_name=collection_name,
            query_properties=query_properties,
            limit=retrieve_limit,
        ...
        response = retriever.vector_search(
            request=query,
            collection_name=collection_name,
            limit=retrieve_limit,
            return_properties=return_properties,
...
def _check_search_type_param(search_type: list) -> None:
    ...
    if not any(search_type):
        raise ValueError(
            f"search_type must contain at least one search type from {accepted_search_types}"
        )
    count = 0
    ...
    if count == 0:
        raise ValueError(
            f"Please use one of {accepted_search_types}. Received {search_type}"
        )


def _add_metrics(
    results_dict: dict, search_type: Literal["kw", "vector", "hybrid", "all"] = ["all"]
) -> dict:
    """
    ...
    for prefix in search_type:
        if prefix in accepted_search_types:
            results_dict = {**results_dict, f"{prefix}_hit_rate": 0, f"{prefix}_mrr": 0}
    return results_dict


def _create_miss_info(
    search_type: str, query: str, response: list[dict], dataset: dict, doc_id: str
) -> dict:
    """
