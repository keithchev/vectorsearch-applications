...
def generate_project2_submission_file(
    eval_dict: dict,
...
async def async_llm_call(
    ...
    """
    user_message = generate_prompt_series(
        query, ranked_result, verbosity_level=verbosity
    )
...
async def main(
    ...
    tasks = [
        async_llm_call(
            llm, query, result, system_message, verbosity, temperature, max_tokens
        )
    ...
    if show_cost:
        ...
        print(
            f"Total cost for {len(ranked_results)} API calls: ${round(total_cost,4)}."
        )
    return responses
...
def sync_llm_calls(
    ...
    for i, result in enumerate(tqdm(ranked_results)):
        ...
        user_message = generate_prompt_series(
            queries[i], result, verbosity_level=verbosity
        )
